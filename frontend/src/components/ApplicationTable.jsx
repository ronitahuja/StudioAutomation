...
const ApplicationTable = ({ payload, setPayLoad }) => {
    ...
    const fetchApplication = async (appName) => {
        ...
        try {
            ...
            if (appData) {
                console.log("fetched connectionLevelParamFields:", appData.connectionLevelParamFields);
                setPayLoad({
    ...
    const handleDelete = (appName) => {
        if (window.confirm(`Are you sure you want to delete "${appName}"?`)) {
            axios.delete(`http://localhost:3000/api/v1/app/${appName}`)
                .then(response => {
                ...
                .catch(error => {
                    console.error(error);
                    alert('An error occurred while deleting the app. Please try again later.');
                });
        }
                                        ...
                                        <tr key={index} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <div className="flex space-x-3">
                                                    ...
                                                    <Trash2 className="h-5 w-5 text-gray-400 cursor-pointer hover:text-gray-600" onClick={() => handleDelete(app.appName)} />
